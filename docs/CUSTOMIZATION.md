...
This guide covers how to customize the product catalog.
...
5. Update the contact details used for product enquiries
...
This config is referenced by the header, footer, and metadata. Change it once, it updates everywhere.
...
Pre-defined catalog events: `search` and `viewProduct`.

## Security Headers

Security headers are configured in `src/middleware.ts`. The Content Security Policy (CSP) uses `report-only` in development and enforces in production.
